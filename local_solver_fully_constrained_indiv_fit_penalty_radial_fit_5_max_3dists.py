...
# Replaced differential_evolution with minimize
...
def cost_function_de_global(params, E, data, T_min, T_max):
    ...
    #residuals = model_vals_safe - data_safe
    #base_cost = np.sum(residuals**2.)
    #3) Build a weighting array w(E)
    #    For example, weigh tail bins more heavily:
    E_tail_threshold = 10.0  # eV, or any threshold you choose
    tail_weight      = 10.0  # how much more you emphasize the tail
   
    w = np.ones_like(E)       # core region weight = 1.0
    w[E >= E_tail_threshold] = tail_weight  # tail region weight
   
    # 4) Weighted sum of squared residuals
    weighted_residuals_sq = w * (residuals**2)
    base_cost = np.sum(weighted_residuals_sq)
    ...
    # median_err  = np.median(pct_err_arr)   
    
    # define penalty 
    alpha = 5e-4
    #alpha_scaled = alpha
    #penalty = alpha * (max_err**2.)
    
    threshold   = 10.0
    #excess      = max(0.0, max_err - threshold)
    #penalty     = alpha * (excess**2)
...
def fit_five_maxwellians(E, data, T_input, T_min, T_max):
    ...
    """
    def b0_b1_constraint(x):
        # b1 - b0 >= 0
        return x[6] - x[5]
    
    def b1_b2_constraint(x):
        # b2 - b1 >= 0
        return x[7] - x[6]
    
    def b2_b3_constraint(x):
        # b3 - b2 >= 0
        return x[8] - x[7]
    
    def b3_b4_constraint(x):
        # b4 - b3 >= 0
        return x[9] - x[8]
    
    constraints = [
        {'type': 'ineq', 'fun': b0_b1_constraint},
        {'type': 'ineq', 'fun': b1_b2_constraint},
        {'type': 'ineq', 'fun': b2_b3_constraint},
        {'type': 'ineq', 'fun': b3_b4_constraint},
    ]
    # Then you pass these to scipy.optimize.minimize(..., constraints=constraints, ...)
    """
    ...
    # Build the bounds
    #bounds = [(-5.,5.)]*5 + [(np.log(T_input/2.), np.log(500.0))]*5
    bounds = [(-10.,10.)]*4 + [(-10.,10.)]*5
    
    # Example initial guess:
    # a_i ~ 0, b_i ~ ln(1) -> T=1 eV 
    # You can adjust if needed
    T_init = np.logspace(np.log10(T_input), np.log10(500), 5)
    #b_init = np.log(T_init)
    p_init = inverse_stick_breaking(T_init, T_min, T_max)
    #c_init = np.diff(b_init, prepend=0)
    #x0 = [0.0]*5 + [np.log(T_input)]*5
    #a[0]=0. # by choice as they are defined u to constants so only fit 4 not 5 now
    # In same way could have done f[0] = 1-rest
    #a_init=[ -2.1806, -4.482, -5.175, -6.785]
    a_init=[ 0.]*4
    #x0 = [0.0]*5 + list(p_init)
    x0 = list(a_init) + list(p_init)
    # Local solve
    result = minimize(
        fun=cost_function_de_global,
        x0=x0,
        args=(E, data, T_min, T_max),
        method='powell',#'L-BFGS-B',
        bounds=bounds,#constraints=constraints,
        options={'maxiter': 20000, 'disp': False, 'ftol': 1e-12,'xtol':1e-12 }
    )
...
if __name__=="__main__":
    ...
    # Save
    np.savez(
        "local_solver_10x10_T_vs_kappa_indiv_dif_ev_fully_contrained_fits_of_Evals.npz",
        T=T,
        kappa=kappa,  
        E_vals=E_vals,
    ...
    for j in range(len(kappa)):
        ...
        plt.tight_layout()
        plt.savefig(f"local_solver_10x10_T_kappa_Ti_fi_subplots_5MaxFits_kappa={kappa[j]:.2f}.pdf", bbox_inches='tight')
        plt.show()
        ...
        plt.tight_layout()
        plt.savefig(f"local_solver_10x10max_rel_error_subplots_5MaxFits_kappa={kappa[j]:.2f}.pdf", bbox_inches='tight')
        plt.show()
