...
def main():
    ...
    # Species (skipping 'oph','eh' for the first 3 distributions):
    ...
    sp = "elec"
    n_in   = n_data["elec"] + n_data["eh"]
    with np.errstate(divide='ignore', invalid='ignore'):
        feh_in =  n_data["eh"]/n_in 
    ...
    n_rz   = interpolate_rhoz(rhof, zff, n_in,   rho1D_unique, z1D_unique)
    feh_rz   = interpolate_rhoz(rhof, zff, feh_in,   rho1D_unique, z1D_unique)
    #Tpar_rz= interpolate_rhoz(rhof, zff, Tpar_in,rho1D_unique, z1D_unique)
    #Tperp_rz=interpolate_rhoz(rhof, zff, Tperp_in,rho1D_unique, z1D_unique)
    #kpar_rz= interpolate_rhoz(rhof, zff, kpar_in,rho1D_unique, z1D_unique)
    #kperp_rz=interpolate_rhoz(rhof, zff, kperp_in,rho1D_unique, z1D_unique)

    # Replicate in phi => shape(241, nphi, 53) => (241,11,53)
    n_3D_sp      = replicate_phi(n_rz,     Nphi)
    feh_3D_sp    = replicate_phi(feh_rz,     Nphi)
    ...
    interpolators_feh[sp] = RegularGridInterpolator(
        (rho1D_unique, phi1D_unique, z1D_unique),
        feh_3D_dict[sp], method='linear', bounds_error=False, fill_value=np.nan
    )
    """
    ...
    i_feh    = interpolators_feh[sp](pts_3D)
    ...
    feh_final[sp]    = i_feh.reshape((Nx,Nz,Ns))
    ...
    # Save final results
    outfilename = "los_results_netot_feh_maxwellian.npz"
    ...
    save_dict["n_elec_tot"]      = n_final[sp]
    save_dict["feh"]          = feh_final[sp]
