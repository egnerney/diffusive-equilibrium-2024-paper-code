function diff_eq_eddie_ionosphere,Pos_in,Pos0_in,Bratio,species0_in,phi,$
...
;print,species0[4].n
species0[nspec-1].n=$
        -total(species0[0:nspec-2].q*species0[0:nspec-2].n , /double)$
...
U0 = -GM/RP*(RJ/Pos0_in[0])-0.5d*RP^2*(Pos0_in[0]/RJ)^2*$
	Cos(Pos0_in[1]*!dtor)^2*Omega^2
U  = -GM/RP*(RJ/Pos_in[0]) -0.5d*RP^2*(Pos_in[0]/RJ)^2 *$
	Cos(Pos_in[1]*!dtor )^2*Omega^2
...
;if (torus_side) then begin
	...
	while (nq*nq2 GT 0d) do begin
		...
		for i=0,nspec-1 do n[i] = call_function( $
		  ...
		  nq_temp = [species0[0].q*n[0],species0[1].q*n[1]] ; removing hot electrons from this
		...
		for i=0,nspec-1 do n2[i] = call_function( $
                ...
                nq2_temp = [species0[0].q*n2[0],species0[1].q*n2[1]]   ; removing hot electrons from this
                nq2 = total(nq2_temp , /double);total(species0.q*n2)
	endwhile

	while (abs(nq/n[nspec-1]/species0[nspec-1].q) GT 1d-5) do begin
		...
		for i=0,nspec-1 do nm[i] = call_function( $
                   ...
                   nqm_temp = [species0[0].q*nm[0],species0[1].q*nm[1]]; removing hot electrons from this
