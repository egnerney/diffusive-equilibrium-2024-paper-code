pro read_in_convolved_frames_sample_o4_eq_for_APO_like_carl_interped_higher_res
...
nzhighres = floor(5.d/0.01d) + 1 
z_higher_res = 0.01*findgen(nzhighres) - 2.5d

nrhohighres = floor(20.d/0.01d) + 1
rho_higher_res = 0.01*findgen(nrhohighres) - 10.d

xgrid1d_old = xgrid1d
zgrid1d_old = zgrid1d

xgrid1d = rho_higher_res
zgrid1d = z_higher_res

x_half_step_sizegrid1d = replicate(0.01,n_elements(xgrid1d))
z_half_step_sizegrid1d = replicate(0.01,n_elements(zgrid1d))
...
B_frame = griddata(rho1d_interpfrom, z1d_interpfrom,B_frame,xout = rho_higher_res, yout = z_higher_res ,/grid,/linear,triangles = tr )
