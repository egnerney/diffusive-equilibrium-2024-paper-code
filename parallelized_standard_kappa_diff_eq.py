...
def maxwellian_density(species, deltaU, Phi, Bratio):
    """
    Computes the density for a Maxwellian distribution. f \propto Exp[-(m*vperp^2 + m*vpar^2)/(2*T)]

    Parameters:
    - species: The species object containing properties like n, m, q, T
    - deltaU: The difference in gravitational-centrifugal potential energy per unit mass (excluding electrostatic potential)
...
def aniso_maxwellian_density(species, deltaU, Phi, Bratio):
    """
    Computes the density for an anisotropic Maxwellian distribution. f \propto Exp[-(m*vperp^2/(2*Tperp) + m*vpar^2/(2*Tpar))]

    Parameters:
    - species: The species object containing properties like n, m, q, T, A
    - deltaU: The difference in gravitational-centrifugal potential energy per unit mass (excluding electrostatic potential)
    - Phi: Electrostatic potential difference
    - Bratio: The ratio B / B0
    """
...
def aniso_kappa_density(species, deltaU, Phi, Bratio):
    """
    Computes the density for an anisotropic kappa distribution. f \propto (1+m*vperp^2/(2*kappa*Tperp) + m*vpar^2/(2*kappa*Tpar))^(-(1 + kappa))

    Parameters:
    - species: The species object containing properties like n, m, q, T, A, kappa
    - deltaU: The difference in gravitational-centrifugal potential energy per unit mass (excluding electrostatic potential) between s and s0
    - Phi: Electrostatic potential difference between s and s0
    - Bratio: The ratio B / B0
    """
    B0_over_B = 1.0 / Bratio  # B0 / B
    ...
    if PEratio <= -1.:
        #print("Warning: Density integral doesn't converge if deltaU/(kappa_par*T_par) <= -1, and gives 0 or negative density")
...
# AnisotropicProduct Kappa density function, unCorrelated Case
def aniso_product_kappa_density(species, deltaU, Phi, Bratio):
    """
    Computes the density for an anisotropic kappa distribution. f \propto (1+m*vperp^2/(2*kappa_perp*Tperp))^(-(1 + kappa_perp))* (1+m*vpar^2/(2*kappa_par*Tpar))^(-(1 + kappa_par))
...
def process_field_line(args):
    (j, species0_in, planet, fcor, if_psh, x_fl, y_fl, z_fl, rho_fl, lat_fl, phi_fl_wlong, r_fl, Btot_fl, r_00,
     ...
     ti0, thp0, tec0, teh0, toph0, kappa_Te, kappa_Top, kappa_e, kappa_op, kappa_Thp) = args
    ...
    if (r_fl_ceq >= 4.00) and (r_fl_ceq <= 10.0):
        ...
        #Ticnew = interp1d(r_00, ti0, fill_value="extrapolate")(r_fl_ceq)
        Thpnew = interp1d(r_00, kappa_Thp, fill_value="extrapolate")(r_fl_ceq)
        #Thpnew = interp1d(r_00, thp0, fill_value="extrapolate")(r_fl_ceq)
        Tecnew = interp1d(r_00, kappa_Te, fill_value="extrapolate")(r_fl_ceq)
       # Tecnew = interp1d(r_00, tec0, fill_value="extrapolate")(r_fl_ceq)
       # Tehnew = interp1d(r_00, teh0, fill_value="extrapolate")(r_fl_ceq)
       # Tophnew = interp1d(r_00, toph0, fill_value="extrapolate")(r_fl_ceq)
        valA = 1.
        Aic = valA #Ticnew / Ticold  # perp/par = kappa/max for fried egg
        Ahp = valA #Thpnew / Thpold
        Aec = valA#Tecnew / Tecold
        Aeh = valA
        Aoph = valA
        Ticnew = Ticnew/Aic
        Thpnew = Thpnew/Ahp
...
if __name__ == '__main__':
    ...
    species_lam = [1.] * nspec
    species_type = ['Aniso_kappa'] * nspec#['Aniso_product_kappa'] * nspec
    ...
    np.savez_compressed('new_nominal_model_n_out_nominal_model_4-10_iso_standard_kappa_all.npz', **n_out)

    # Save T_out (dictionary of species temperatures)
    np.savez_compressed('new_nominal_model_T_out_nominal_model_4-10_iso_standard_kappa_all.npz', **T_out)

    # Save x_out, y_out, z_out, B_out (NumPy arrays)
    np.savez_compressed('new_nominal_model_field_data_nominal_model_4-10_iso_standard_kappa_all.npz',
