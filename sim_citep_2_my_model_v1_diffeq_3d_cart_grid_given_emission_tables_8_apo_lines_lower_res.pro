...
;;;;;;;;;;;spacer
function calc_LOS_n_s_and_Tel_s_given_my_model_3D_cart_grid_double, xgrid, ygrid, zgrid, norm_vec, slit_pos_vec, n_in, T_in, s=s, tel=tel
...
;help,rho_los
  ...
  ;tic
     ...
     ; jjj=[jjj,jj]
     ; kkk=[kkk,kk]
    ;  print,xgrid(ii),x_los(a)
     ; print,ygrid(jj),y_los(a)
     ; print,zgrid(kk),z_los(a)
      ;stop
 ...
 ;print,min(x_los - xgrid(iii)),max(x_los - xgrid(iii)),median(x_los - xgrid(iii))
 ;print,min(y_los - ygrid(jjj)),max(y_los - ygrid(jjj)),median(y_los - ygrid(jjj))
 ;print,min(z_los - zgrid(kkk)),max(z_los - zgrid(kkk)),median(z_los - zgrid(kkk))
  
 ;stop
 ;p1=plot(s,n.el)
 ;p2=plot(s,n_in[229,*,64].el,/overplot,color='red')
 ;stop
 ;help, n.el
  ;stop
...
Function new_CITEP_2_given_n_s_and_tel_s_using_emission_tables_yptsi_double,s,n,Tel_s,yptsi_,nel_,tec_
...
;stop
 ...
 ;yptsi(j)=7149.2d*tsum(s, reform(B_intgrnd_values(*,j)))
...
Pro sim_citep_2_my_model_v1_diffeq_3D_cart_grid_given_emission_tables_8_APO_lines_lower_res
   ...
   Restore, 'Mymodelv1_3D_JRM33+con20_cartesian_irrcart_0.025res_mapped_no_interp.sav',/verbose

   dx1 = 0.1d
   nx1 = 40

   dx2 = 0.05d
   nx2 = 76;380 is for 0.01 ; 152 is 0.025 ; 76 for 0.05

   nx3 = 44


   xgrid1d = [ dx1*dindgen(nx1) - 10.d , dx2*dindgen(nx2) - 6.d , dx1*dindgen(nx3) - 2.2d , dx2*dindgen(nx2) + 2.2d , dx1*dindgen(nx1+1) + 6.d ]
   ...
   z_min = -2.5d;
   z_max = 2.5d;
   z_step_size = 0.05d ;  RJ
   n_z = Round((z_max - z_min)/z_step_size) + 1
   zgrid1d = z_step_size*dindgen(n_z) + z_min

   z_half_step_sizegrid1d = replicate(z_step_size/2.d,n_z)
...
;p1=jade_spectrogram(reform(n_in[*,138,*].el),xgrid,x_half_step_sizegrid1d,zgrid,z_half_step_sizegrid1d)

 ;stop
 
 ;p1=jade_spectrogram(reform(T_in[*,138,*].el),xgrid,x_half_step_sizegrid1d,zgrid,z_half_step_sizegrid1d)
;print,n_in[229,138,64].el,t_in[229,138,64].el
;stop
xmin=550.d;3000d;
xmax=2100.d;7000d;
xstepsize=1.d
...
;euvbin=0.6049d ;euv binsize cassini UVIS
 ...
 ypts_vary_x0_and_z0=dblarr(n_elements(xgrid),n_elements(zgrid), 8)
   ...
   ; p1=jade_spectrogram(reform(yptsi_in(*,*,0)),nel,nelhalf,tec,techalf,xrange=[2000,4000],yrange=[2.0,5.0])
;;;;;;;;;;;
    ;stop
...
For i = 0, n_elements(xgrid)-1 do begin ; for x= 4.5 to 10
  For j = 0, n_elements(zgrid)-1 do begin ; for z=0 to z= 3
    ...
    x0=xgrid(i);xgrid(229);4.6d;xgrid(220) issue ;229,138,64
    z0=zgrid(j);zgrid(64);zgrid(84) issue
    ;y0=-10
     print,x0,y0,z0
    slit_pos_vec=[x0,y0,z0]
...
;tic
    ...
    n = calc_LOS_n_s_and_Tel_s_given_my_model_3D_cart_grid_double( xgrid, ygrid, zgrid, norm_vec, slit_pos_vec, n_in, T_in, s=s, tel=tel)
  ...
  ; print,n_elements(tel),n_elements(s),n_elements(n.el)
   ;p1=plot(s,n.el,ylog=1)
   ;p2=plot(s,n_in[229,*,64].el,ylog=1,/overplot,color='red')
   ;p3=plot(s,tel,ylog=1)
   ;p4=plot(s,t_in[229,*,64].el,ylog=1,/overplot,color='red')
   ; stop
  ;  tic
    ypts=new_CITEP_2_given_n_s_and_tel_s_using_emission_tables_yptsi_double(s,n,Tel,yptsi_in_2dto1d,nel2dto1d,tec2dto1d)
    ;print,imin,jmin
    ;stop
   ...
   ypts_total_vary_x0_and_z0(i,j)=total(ypts,/double);,/double)
...
; xrange1=[xmin,xmax]
  ...
  ;write_csv,'ypts_8apolinesa_vary_x0_and_z0_emission_table_277x101x8_with_tsum.txt',ypts_vary_x0_and_z0
  ;  write_csv,'ypts_total_8apolines_vary_x0_and_z0_emission_table_277x101_with_tsum.txt',ypts_total_vary_x0_and_z0
p1=jade_spectrogram(ypts_total_vary_x0_and_z0,xgrid,x_half_step_sizegrid1d,zgrid,z_half_step_sizegrid1d)
p1=jade_spectrogram(ypts_vary_x0_and_z0(*,*,7),xgrid,x_half_step_sizegrid1d,zgrid,z_half_step_sizegrid1d)
print,max(ypts_vary_x0_and_z0(*,*,7))
