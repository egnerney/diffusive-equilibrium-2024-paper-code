function diff_eq_eddie_iono_and_magneto,Pos_in,Pos0_in,Bratio,Pos0i_in,potdrop_in,species0_in,phi,$
...
if (n_elements(RJ) EQ 0) then RJ = 7.14e7
...
species0.m *= 1.67d-27
species0.q *= 1.60217d-19
species0.T *= 1.60217d-19
...
;print,species0[4].n
species0[nspec-1].n=$
        -total(species0[0:nspec-5].q*species0[0:nspec-5].n , /double)$
        /species0[nspec-1].q       ;   
...
U0 = -GM/RP*(RJ/Pos0_in[0]) - 0.5d*(RP^2d)*((Pos0_in[0]/RJ)^2d)*(Cos(Pos0_in[1]*!dtor)^2d)*(Omega^2d)

U0i = -GM/RP*(RJ/Pos0i_in[0]) - 0.5d*(RP^2d)*((Pos0i_in[0]/RJ)^2d)*(Cos(Pos0i_in[1]*!dtor)^2d)*(Omega^2d)	
	
U  = -GM/RP*(RJ/Pos_in[0]) - 0.5d*(RP^2d)*((Pos_in[0]/RJ)^2d)*(Cos(Pos_in[1]*!dtor )^2d)*(Omega^2d)
...
;		for i=0,nspec-1 do begin
	...
	;	   if ((i eq 9) or (i eq 8)) then n[i] = species0[i].n*exp((species0[i].m*(U0i - U) - species0[i].q*(Phi - potdrop_in))/species0[i].T)
...
;		  nq_temp = [species0[0:nspec-3].q*n[0:nspec-3],species0[nspec-1].q*n[nspec-1]] ; removing hot electrons from this
  ...
  ;               if ((i eq 9) or (i eq 8)) then n2[i] = species0[i].n*exp((species0[i].m*(U0i - U) - species0[i].q*(Phi2 - potdrop_in))/species0[i].T)
  ...
  ;              nq2 = total(nq2_temp , /double);total(species0.q*n2)
	...
	Phi = -5d*abs(potdrop_in)
	
	Phi2 = 5d*abs(potdrop_in)
	i=8
nhpidrop = species0[i].n*exp((species0[i].m*(U0i - U0) - species0[i].q*( - potdrop_in))/species0[i].T)
	i=9
	neidrop = species0[i].n*exp((species0[i].m*(U0i - U0) - species0[i].q*( - potdrop_in))/species0[i].T)
print, nhpidrop,neidrop, nhpidrop/neidrop 
	stop
	
	print,'here2'

	while (abs(nq/(n[nspec - 1] + n[nspec - 3])/species0[nspec-1].q) GT 1d-5) do begin
     ...
     if ((i eq 9) or (i eq 8)) then nm[i] = species0[i].n*exp((species0[i].m*(U0i - U) - species0[i].q*(Phim - potdrop_in))/species0[i].T)
   endfor 
