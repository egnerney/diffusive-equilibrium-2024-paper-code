...
;         
;        V.33, 13-Feb-2018, Edward G. Nerney
;               Added proton_ratio input to emiss_calc to explicitly 
;               define the proton to electron ratio. Also explicitly set Status=-1 
;               so no ionization or recombination corrections to the populations are used.
;                
;
;
;
...
FUNCTION  EMISS_CALC_EGN, IZ, ION, TEMP=TEMP, DENS=DENS, RADTEMP=RADTEMP, $
                      RPHOT=RPHOT, $
                      ...
                      RADFUNC=RADFUNC, NO_CALC=NO_CALC, proton_ratio=proton_ratio
...
;explicitly set Status=-1 
;so no ionization or recombination corrections to the populations are used.

Status=-1 ; 

; Added proton_ratio input to emiss_calc to
; explicitly define the proton to electron ratio.
pe_ratio=proton_ratio 
...
; from filename.
;
; zion2filename is called on to help locate the relevant CHIANTI database files. 
;
; zion2spectroscopic is called on to provide identification strings
;
...
; If you use pressure and temp as inputs instead of temp and density then N_ELEMENTS(pressure) NE 0
; In this case the code will find the density using the ideal gas law:
...
; This first call is only used to find the dimensions of the output structure. 
...
;The following sets the multiplicative factor to multiply N_j by. Either hc/lamb A_ji or just A_ji
...
; The code below calls on pop_solver to solve the level balance equations and return N_j

; N_ELEMENTS(pressure) NE 0 then you are using the pressure instead of the density as an input


; N_ELEMENTS(pressure) = 0 then you are using the density instead of the pressure as an input
...
;pop_solver above finds N_j and then these are multiplied by hc/lamb A_ji 
; or just A_ji depending on no_de keyword  
;These emissivities are stored in emiss.em
