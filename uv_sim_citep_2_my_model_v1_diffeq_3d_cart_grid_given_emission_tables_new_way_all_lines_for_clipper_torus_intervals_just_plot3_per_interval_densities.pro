...
Pro UV_sim_citep_2_my_model_v1_diffeq_3D_cart_grid_given_emission_tables_new_way_all_lines_for_clipper_torus_intervals_just_plot3_per_interval_densities
...
for iii = 0, n_intervals - 1 do begin
  for jjj_proxy = 0, 2 do begin;n_et_each_interval - 1 do begin
    ...
    jjj = indicies_for_jjj_proxy[jjj_proxy]
     print,'iii = ', iii, ' of 44'
    print,'jjj = ', jjj, ' of 1800'
  norm_vec = reform( BS_pointing_vector_Clipper_UVS_AP[*,jjj,iii]); [0d,1d,0d];
    slit_pos_vec = [x_SC[jjj,iii],y_SC[jjj,iii],z_SC[jjj,iii]];[x0t,y0t,z0];[x0,y0,z0] ; [6d,-10d,0d];
    n = calc_LOS_n_s_and_Tel_s_given_my_model_3D_cart_grid_diffthreee_for_faraway( xgrid, ygrid, zgrid, norm_vec, slit_pos_vec, n_in, T_in, s=s, tel=tel)
     ...
     utc_outz(iii,jjj_proxy) = utc_sc[jjj,iii]
    s_outz(iii,jjj_proxy,0:n_elements(s)-1) = s
    
    norm_vec_outz(*,iii,jjj_proxy) = norm_vec
     slit_pos_vec_outz(*,iii,jjj_proxy) = slit_pos_vec
    
    x_outz(iii,jjj_proxy,0:n_elements(s)-1) = slit_pos_vec[0] + s*norm_vec[0]
    y_outz(iii,jjj_proxy,0:n_elements(s)-1) = slit_pos_vec[1] + s*norm_vec[1]
    z_outz(iii,jjj_proxy,0:n_elements(s)-1) = slit_pos_vec[2] + s*norm_vec[2]
    
    tec_outz(iii,jjj_proxy,0:n_elements(s)-1) = tel
    nec_outz(iii,jjj_proxy,0:n_elements(s)-1) = n.el
    nsp_outz(iii,jjj_proxy,0:n_elements(s)-1) = n.sp
    ns2p_outz(iii,jjj_proxy,0:n_elements(s)-1) = n.s2p
    ns3p_outz(iii,jjj_proxy,0:n_elements(s)-1) = n.s3p
    nop_outz(iii,jjj_proxy,0:n_elements(s)-1) = n.op
    no2p_outz(iii,jjj_proxy,0:n_elements(s)-1) = n.o2p
