# -*- coding: utf-8 -*-
"""
Created on Mon Nov 25 01:06:08 2024
...
if __name__ == '__main__':
    ...
    for case_idx, case_name in enumerate(case_names):
        ...
        n_points = len(flux_tube_content_array_sorted)
        polyorder = 2
        # Ensure window_length is odd and does not exceed the data length
        window_length = min(13, n_points if n_points % 2 == 1 else n_points - 1)
        # Ensure window_length is at least polyorder + 2
        min_window_length = polyorder + 2
        ...
        # Apply the filter
        flux_tube_content_filtered = savgol_filter(
            flux_tube_content_array_sorted,
            window_length=window_length,
            polyorder=polyorder
        )

        # Plot the filtered data
        plt.plot(r_eq_array_sorted, flux_tube_content_filtered, label=case_labels[case_idx])
        
    legend_properties = {'weight':'bold'}
    # Customize the plot
    plt.xlabel(r'Equatorial Radial Distance $\bf{L}$ ($\bf{R_J}$)', fontsize=14, fontweight='bold')
    plt.ylabel('Total Flux Tube Content (cm)', fontsize=14, fontweight='bold')
    plt.title('Total Flux Tube Content vs Equatorial Radial Distance', fontsize=16, fontweight='bold')
    ...
    # Save the figure
    plt.savefig(fname='total_flux_tube_content_vs_L_logscale.pdf', dpi=600, bbox_inches='tight')
    plt.show()
