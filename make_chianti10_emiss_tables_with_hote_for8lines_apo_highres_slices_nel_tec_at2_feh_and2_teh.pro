...
function calculate_IPT_emiss_citep2_output_yname_emiss_calc_given_feh_teh, Tec, nel,feh,Teh, Nsp, Ns2p, Ns3p,Ns4p, Nop, No2p, $
  min = min, max = max, xwavi=xwavi, yname = yname, wflag=wflag
  ...
  wflag = [s2em.flag, s3em.flag, s4em.flag,s5em.flag, o2em.flag, o3em.flag]
  wflag = wflag[wsort]
...
Pro make_CHIANTI10_emiss_tables_with_hote_for8lines_apo_highres_slices_nel_tec_at2_feh_and2_teh
...
feh = [0.0025d,0.01d];[0.0001d,0.001d,0.0025d,0.005d,0.01d,0.05d,0.1d]
Teh = [45d,300d];[5d*dindgen(4) + 35d, 20d*dindgen(3) + 60d, 100d*dindgen(3) + 200d]
...
yptsi_vary_nel_tec_feh_teh = dblarr(n_ne, n_feh, n_teh, nxwavi)
...
;print,'k = ',k
  ...
  fehk = feh(k)
  tehl = teh(l)
 ;Tec, nel,feh,Teh, Nsp, Ns2p, Ns3p,Ns4p, Nop, No2p, $
  ;min = min, max = max, xwavi=xwavi, yname = yname, wflag=wflag
   ...
   ; stop
   yptsi_vary_nel_tec_feh_teh(i,k,l,*) = yptsi;reform(yptsi,1,1,n_elements(  yptsi))

   
   
   ;help,xwavi,yptsi,wflag
...
APO_emiss_table = dblarr(n_ne, n_feh, n_teh, 8)
...
yptsi_in_temp = reform(yptsi_in(28,0,0,species_want))
...
write_csv,'yptsi_8_APO_LINES_optical_vary_nec_tec_feh_teh_lookuptable_nel_slice_Tec=5eV_61x2x2x8_CHIANTI_10.1.dat',APO_emiss_table



yptsi_vary_nel_tec_feh_teh = dblarr(n_te, n_feh, n_teh, nxwavi)
;for i=0,n_ne -1 do begin
  ;print,'i = ',i
  for j=0, n_te -1 do begin
  print,'j = ',j
  for k=0, n_feh -1 do begin
    ...
    for l=0, n_teh -1 do begin
      ...
      ;print,n_elements(  yptsi)
      ; stop
      yptsi_vary_nel_tec_feh_teh(j,k,l,*) = yptsi;reform(yptsi,1,1,n_elements(  yptsi))
...
APO_emiss_table = dblarr(n_te, n_feh, n_teh, 8)
...
yptsi_in_temp = reform(yptsi_in(17,0,0,species_want))
print,xwavi_inn(idxxx)
print, yptsi_in_temp(idxxx)/max(yptsi_in_temp(idxxx))
...
write_csv,'yptsi_8_APO_LINES_optical_vary_nec_tec_feh_teh_lookuptable_Tec_slice_nel=2000percc_57x2x2x8_CHIANTI_10.1.dat',APO_emiss_table
