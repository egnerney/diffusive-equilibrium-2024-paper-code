# -*- coding: utf-8 -*-
"""
Created on Thu Apr 17 23:56:36 2025
...
def cost_func(params, E, data, T_min, T_max):
    model = five_maxwellians(E, params, T_min, T_max)
    EPS   = 1e-300                        # ≥ exp(‑700)
...
def fit_five_maxwellians(E, data, T_input, T_min, T_max):
    ...
    de_bounds = list(zip(lower, upper))     # list of (lo,hi) tuples
   ...
   # q_init = f_to_q(f_init) 
    ...
    result = differential_evolution(
        ...
        strategy='rand1bin',
        popsize=50,
        init='sobol',          # use our tailored Sobol seed
        maxiter=40000,
        tol=1e-5,
        polish=True,
        workers=-1,
    ...
    #print("Global DE converged:", result.success,
    #     "|  cost =", best_cost,
    #     "|  nfev =", result.nfev)
...
if __name__=="__main__":
    ...
    for i in range(len(T)):
        ...
        for j in range(len(kappa)):
            print(" i = ",i, f" of {num_points_T-1}, j = ",j, f" of {num_points_kappa-1}")
            ...
            for k in range(4):
                ...
                if (f_i_max4_sk[k, i, j] < f_i_max4_sk[k + 1, i, j] - 1e-7):
                    print(f"Standard kappa fi Constraints not Met for f {k}")
                if (f_i_max4_pk[k, i, j] < f_i_max4_pk[k + 1, i, j] - 1e-7):
                    print(f"Product kappa fi Constraints not Met for f {k}")
                if (f_i_max4_fe[k, i, j] < f_i_max4_fe[k + 1, i, j] - 1e-7):
                    print(f"Fried-Egg fi Constraints not Met for f {k}")

    # Save
    np.savez(
        "diffev_fixed_order_fi_and_Ti_local_solver_10x10_T_vs_kappa_indiv_over_Elog150pnts_0.01-min_of_500_or_100xT.npz",
        T=T,
        kappa=kappa,  
        kappa2=kappa2,
        E_vals=E_vals,
    ...
    for j in range(len(kappa)):
        ...
        plt.tight_layout()
        plt.savefig(f"diffev_fixed_order_fi_and_Ti_local_solver_10x10_T_kappa_Ti_fi_subplots_5MaxFits_kappa={kappa[j]:.2f}_kappa2={kappa2[j]:.2f}_Elog150pnts_0.01-min_of_500_or_100xT.pdf", bbox_inches='tight')
        plt.show()
        ...
        plt.tight_layout()
        plt.savefig(f"diffev_fixed_order_fi_and_Ti_local_solver_10x10max_rel_error_subplots_5MaxFits_kappa={kappa[j]:.2f}_kappa2={kappa2[j]:.2f}_Elog150pnts_0.01-min_of_500_or_100xT.pdf", bbox_inches='tight')
        plt.show()
