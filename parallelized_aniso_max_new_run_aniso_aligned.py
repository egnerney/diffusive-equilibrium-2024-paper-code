...
def process_field_line(args):
   ...
   """
    ...
    if (r_fl_ceq >= 4.00) and (r_fl_ceq <= 10.0):
        ...
        Tophot = interp1d(r_00, toph0, fill_value="extrapolate")(r_fl_ceq)
        ...
        Tehot = interp1d(r_00, teh0, fill_value="extrapolate")(r_fl_ceq)

        Aic = 1. #Ticnew / Ticold  # perp/par = kappa/max for fried egg
        Aoph = Aic #Tophnew / Tophot
        Ahp = Aic #Thpnew / Thpold
        Aec =  Aic #Tecnew / Tecold
        Aeh =  Aic #Tehnew / Tehot
        Ticnew = Ticold.item()
        Thpnew = Thpold.item()
        Tecnew = Tecold.item()
        Tehnew = Tehot.item()
        ...
        # Placeholders as 1 as Maxwellians
        lamic = 1.
        lame=1.
        lamhp = 1.
        T_values = [Ticnew,  # O+
        ...
        # Placeholder as Maxwellians
        kappainew = 1. #kappainew.item()
        kappaenew = 1. #kappaenew.item()
...
if __name__ == '__main__':
    ...
    np.savez_compressed('final_new_nominal_model_n_out_nominal_model_4-10_isoT_A=1_max_all_nominal_model.npz', **n_out)

    # Save T_out (dictionary of species temperatures)
    np.savez_compressed('final_new_nominal_model_T_out_nominal_model_4-10_isoT_A=1_max_all_nominal_model.npz', **T_out)

    # Save T_out (dictionary of species temperatures)
    np.savez_compressed('final_new_nominal_model_kappa_out_nominal_model_4-10_isoT_A=1_max_all_nominal_model.npz', **kappa_out)


    # Save x_out, y_out, z_out, B_out (NumPy arrays)
    np.savez_compressed('final_new_nominal_model_field_data_nominal_model_4-10_isoT_A=1_max_Tperp_all_nominal_model.npz',
