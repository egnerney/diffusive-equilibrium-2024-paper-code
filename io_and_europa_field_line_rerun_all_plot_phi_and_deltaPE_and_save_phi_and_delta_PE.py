...
def process_field_line(case_name, field_line_index, species0_in, planet, fcor, if_psh, x_fl, y_fl, z_fl, rho_fl, lat_fl, phi_fl_wlong, r_fl, Btot_fl, r_00,
    ...
    if (r_fl_ceq >= 4.00) and (r_fl_ceq <= 10.0):
        ...
        species_list = setup_species_for_case(case_name, species0_in, Ticnew, Thpnew, Tecnew, r_fl_ceq, r_00, nop0, no2p0, nsp0, ns2p0, ns3p0,
                                              nhp0, nnap0, noph0, nec0, neh0, kappa_e, kappa_op, kappa_Te, kappa_Top, kappa_Thp)
...
def setup_species_for_case(case_name, species0_in, Ticnew, Thpnew, Tecnew, r_fl_ceq, r_00, nop0, no2p0, nsp0, ns2p0, ns3p0,
                           nhp0, nnap0, noph0, nec0, neh0, kappa_e, kappa_op, kappa_Te, kappa_Top, kappa_Thp):
    ...
    for sp in species_list:
        if sp.name == 'O+':
            ...
            sp.n = nophnew
            sp.T = 362.0  # Example value
            sp.kappa = kappa_op_new
            sp.lam = 1.0
        elif sp.name == 'eh-':
            sp.n = nehnew
            sp.T = 46.0  # Example value
...
if __name__ == '__main__':
    ...
    # Plot Phi vs s (overlaid for all 6 cases) for field lines 190 and 539
    ...
    for idx, field_line_index in enumerate(field_line_indices):
        ...
        ax.set_xlabel(r's ($R_J$)', fontsize=14, fontweight='bold')
        ax.set_title(f'Electrostatic Potential along {field_line_labels[field_line_index]}', fontsize=16, fontweight='bold')
        ax.grid(True)
        ax.tick_params(axis='both', which='major', labelsize=12)
        if idx == 0:
            ax.set_ylabel('Electrostatic Potential $\Phi$ (V)', fontsize=14, fontweight='bold')
        if idx == 1:
    ...
    # Plot total potential energy difference in eV for all species, in multiplot panels for all 6 cases
    species_labels = ['O$^{+}$', 'O$^{++}$', 'S$^{+}$', 'S$^{2+}$', 'S$^{3+}$', 'H$^{+}$', 'Na$^{+}$', 'O$^{+}$(hot)', 'e$^{-}$(hot)', 'e$^{-}$']
    ...
    for field_line_index in field_line_indices:
        ...
        for idx_case, case_name in enumerate(case_names):
            ...
            for i, sp in enumerate(species_list):
                ...
                # Filter out non-positive potential energies for log scale
                #valid_indices = pe_eV_positive > 1e-2
                s_valid = s_positive#[valid_indices]
                pe_eV_valid = pe_eV_positive#[valid_indices]

                ax.plot(s_valid, pe_eV_valid, label=species_labels[i])
            #ax.set_yscale('log')
            #ax.set_ylim(bottom=1e-2)
            ax.set_title(f'Case {case_name}', fontsize=14, fontweight='bold')
            ax.set_ylabel('Total Potential Energy (eV)', fontsize=12, fontweight='bold')
            ax.grid(True, which='both', linestyle='--')
            if idx_case >= 4:
                ax.set_xlabel(r's ($R_J$)', fontsize=12, fontweight='bold')
            if idx_case == 5:
                ax.legend(fontsize=8)
        fig.suptitle(f'Total Potential Energy along {field_label} (Northern Hemisphere)', fontsize=16, fontweight='bold')
