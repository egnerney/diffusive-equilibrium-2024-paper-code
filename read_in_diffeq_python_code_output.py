...
# Load n_out
n_out_loaded_isomax = np.load('new_nominal_model_n_out_new_nominal_model_4-10_iso_max_all.npz')
n_out_isomax = {key: n_out_loaded_isomax[key] for key in n_out_loaded_isomax}

# Load n_out
n_out_loaded = np.load('new_nominal_model_n_out_new_nominal_model_4-10_iso_max_all.npz')
n_out = {key: n_out_loaded[key] for key in n_out_loaded}

# Load T_out
T_out_loaded = np.load('new_nominal_model_T_out_new_nominal_model_4-10_iso_max_all.npz')
T_out = {key: T_out_loaded[key] for key in T_out_loaded}

# Load field data
field_data = np.load('new_nominal_model_field_data_new_nominal_model_4-10_iso_max_all.npz')
...
# Equatorial radius: 1 R_J
# Polar radius: approximately 0.935 R_J
oblateness_factor = 0.935  # Adjust as needed
...
minnoph=0.1
maxnoph=n_out_isomax['oph'].max()

minneh= 2.0 #0.1
maxneh= n_out['eh'].max()#1.0 #n_out['eh'].max()
...
cbar_label_ticks_neh = [ 0.1, 0.5,1.0]
cbar_tick_labels_neh = {tick: str(tick) if tick in cbar_label_ticks_neh else '' for tick in cbar_ticks_neh}
"""
...
for iii in range(10):
    ...
    # Create contour plot using tricontourf
