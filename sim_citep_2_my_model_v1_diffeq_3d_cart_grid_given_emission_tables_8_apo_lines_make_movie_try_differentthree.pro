...
Pro sim_citep_2_my_model_v1_diffeq_3D_cart_grid_given_emission_tables_8_APO_lines_make_movie_try_differentthree
...
nframes = 90;8d ; proof of concept for now...
  ...
  ; i=231 ; x= 5.65
   ;j=92;64 ; z = 0.7, sqrt(x^2 + z^2 ) = 5.6932 ~ 5.7
    ...
    slit_pos_vec = [xgrid(i)*Cos(phi[k]) + 10d*Sin(phi[k]) , xgrid(i)*sin(phi[k]) - 10d*cos(phi[k]) , zgrid(j)];[x0t,y0t,z0];[x0,y0,z0]
       ;print,i,j
   ; print, slit_pos_vec[0], slit_pos_vec[1], slit_pos_vec[2]
...
; xrange1=[xmin,xmax]
  ...
  write_csv,'ypts_8apolinesa_vary_x0_and_z0_emission_table_90x279x129x8_movieframes_trydiff3.txt',ypts_vary_x0_and_z0
    write_csv,'ypts_total_8apolines_vary_x0_and_z0_emission_table_90x279x129_movieframes_trydiff3.txt',ypts_total_vary_x0_and_z0
