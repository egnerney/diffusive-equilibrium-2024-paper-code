...
pro read_in_ceq_radial_o4_APO_simulations_make_pdf_book_linear_interped_for_fits
...
;restore,filename='linear_interpedhigher_res_simulated_apo_8lines_dusk_90_frames_given_1.2xionsfit_90framesx376_ceq_radialbins_8lines.sav',/verbose
restore,filename='linear_interpedhigher_res_simulated_apo_8lines_Dusk_90_frames_given_1.2xionsfit_correction_factors_90framesx376_ceq_radialbins_8lines.sav',/verbose
...
p11=errorplot(x_grid,dusk_grid(*,2),ERR_dusk_grid(*,2),NAME='OII (O+) 3729 Angstroms',ERRORBAR_COLOR="blue",color="blue",title='1.2*Ion Full Correction Factor',xtitle='Dusk $\rho_c$ ($R_J$)',ytitle='Rayleighs',SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01,yrange=[0,400],xrange=[4.5,7])

p22=errorplot(x_grid,dusk_grid(*,6),ERR_dusk_grid(*,6),NAME='SII (S+) 6716 Angstroms',ERRORBAR_COLOR="red",color="red",/overplot,SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01)
p33=errorplot(x_grid,dusk_grid(*,7),ERR_dusk_grid(*,7),NAME='SII (S+) 6731 Angstroms',ERRORBAR_COLOR="brown",color="brown",/overplot,SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01)

p44=errorplot(x_grid,dusk_grid(*,0),ERR_dusk_grid(*,0),NAME='SIII (S++) 3722 Angstroms',ERRORBAR_COLOR="pink",color="pink",/overplot,SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01)
p55=errorplot(x_grid,dusk_grid(*,1),ERR_dusk_grid(*,1),NAME='OII (O+) 3726 Angstroms',ERRORBAR_COLOR="purple",color="purple",/overplot,SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01)

p66=errorplot(x_grid,dusk_grid(*,5),ERR_dusk_grid(*,5),NAME='SIII (S++) 6312 Angstroms',ERRORBAR_COLOR="orange",color="orange",/overplot,SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01)

p77=errorplot(x_grid,dusk_grid(*,3),ERR_dusk_grid(*,3),NAME='SII (S+) 4069 Angstroms',ERRORBAR_COLOR="light Green",color="light Green",/overplot,SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01)
p88=errorplot(x_grid,dusk_grid(*,4),ERR_dusk_grid(*,4),NAME='SII (S+) 4076 Angstroms',ERRORBAR_COLOR="dark green",color="dark green",/overplot,SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01)
...
leg2 = LEGEND(TARGET=[p3,p2,p6,p8,p7,p1,p5,p4], POSITION=[7.1,400.], $
 ...
 ; leg2.save,'0to400rayleigh_Dusk_no_offset_1.2xions_correction_factor_duskcoaddedfits_mean_interped_each_frame_all_apo_lines.png',resolution=300



 p1=plot(x_grid,dusk_grid(*,2)/interpol(mean(reform(B_sim(*,*,2)),dimension = 1),rho_ceq,x_grid),NAME='$O^+$ 3729 Å',color="blue",xtitle='Dusk $\rho_c$ ($R_J$)',ytitle='Ratio',title='Obs/(Model run through process), 1st it',yrange=[0.5,2])

   p2=plot(x_grid,dusk_grid(*,6)/interpol(mean(reform(B_sim(*,*,6)),dimension = 1),rho_ceq,x_grid),NAME='$S^+$ 6716 Å',color="red",/overplot);,SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01)
 p3=plot(x_grid,dusk_grid(*,7)/interpol(mean(reform(B_sim(*,*,7)),dimension = 1),rho_ceq,x_grid),NAME='$S^+$ 6731 Å',color="brown",/overplot);,SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01)

 p4=plot(x_grid,dusk_grid(*,0)/interpol(mean(reform(B_sim(*,*,0)),dimension = 1),rho_ceq,x_grid),NAME='$S^{++}$ 3722 Å',color="pink",/overplot);,SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01)
 p5=plot(x_grid,dusk_grid(*,1)/interpol(mean(reform(B_sim(*,*,1)),dimension = 1),rho_ceq,x_grid),NAME='$O^+$ 3726 Å',color="purple",/overplot);,SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01)

 p6=plot(x_grid,dusk_grid(*,5)/interpol(mean(reform(B_sim(*,*,5)),dimension = 1),rho_ceq,x_grid),NAME='$S^{++}$ 6312 Å',color="orange",/overplot);,SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01)

 p7=plot(x_grid,dusk_grid(*,3)/interpol(mean(reform(B_sim(*,*,3)),dimension = 1),rho_ceq,x_grid),NAME='$S^+$ 4069 Å',color="light Green",/overplot);,SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01)
 p8=plot(x_grid,dusk_grid(*,4)/interpol(mean(reform(B_sim(*,*,4)),dimension = 1),rho_ceq,x_grid),NAME='$S^+$ 4076 Å',color="dark green",/overplot);,SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01)
 ...
 ;leg2.save,'ratio_1.2x_correction_factor_dusk_coadded_obs_over_process_1stit.png',resolution=300




   p2=plot(x_grid,(dusk_grid(*,6)/interpol(mean(reform(B_sim(*,*,6)),dimension = 1),rho_ceq,x_grid) + dusk_grid(*,7)/interpol(mean(reform(B_sim(*,*,7)),dimension = 1),rho_ceq,x_grid))/2d,xtitle='Dusk $\rho_c$ ($R_J$)',ytitle='Ratio',title='Obs/(Model run through process), averaged over $S^+$ doublet',yrange=[0.5,2]);,SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01)

p2.save,'Obs_over_Model_run_through_process_averaged_over_S+_doublet.png',resolution = 300
 
 
 p3=plot(x_grid,(dusk_grid(*,2)/interpol(mean(reform(B_sim(*,*,2)),dimension = 1),rho_ceq,x_grid) + dusk_grid(*,1)/interpol(mean(reform(B_sim(*,*,1)),dimension = 1),rho_ceq,x_grid))/2d,xtitle='Dusk $\rho_c$ ($R_J$)',ytitle='Ratio',title='Obs/(Model run through process), averaged over $O^+$ doublet',yrange=[0.5,2]);,SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01)

  p3.save,'Obs_over_Model_run_through_process_averaged_over_O+_doublet.png',resolution = 300


  p4=plot(x_grid,(dusk_grid(*,0)/interpol(mean(reform(B_sim(*,*,0)),dimension = 1),rho_ceq,x_grid) + dusk_grid(*,5)/interpol(mean(reform(B_sim(*,*,5)),dimension = 1),rho_ceq,x_grid))/2d,xtitle='Dusk $\rho_c$ ($R_J$)',ytitle='Ratio',title='Obs/(Model run through process), averaged over $S^{++}$',yrange=[0.5,2]);,SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01)

    p4.save,'Obs_over_Model_run_through_process_averaged_over_S++.png',resolution = 300
...
restore,'dusk_varyall_layers_after_func_form_only3sp_lines_simult_nec_nsp_fit_11params_int_tab_whole_side_simult_steffl_Tec_then_fit_ns2p_and_nop_7_params.sav',/verbose
...
;restore,FILENAME = 'dusk_funcform_only3sp_lines_simult_nec_nsp_fit_11params_int_tab_whole_side_simult_steffl_Tec_then_fit_ns2p_and_nop_7_params.sav',/verbose

openr,1,'factor_corrections_dusk.csv';'yptsi_8_APO_LINES_optical_vary_ne_te_lookuptable_61x57x8_CHIANTI801.dat'
factor_corrections_prv=dblarr(60,3) ; ne x tec x discrete wavelength centers of each emission line
readf,1,factor_corrections_prv
close,1

model = given_number_LOS_dens_temp_for_each_layer_simulate_rayleighs_dawn_or_dusk_all_at_once_8APO_lines_fixed_proper_nintegration_now(te_fixed, nec_fixed_found,factor_corrections_prv(*,0)*nsp_fixed_found,factor_corrections_prv(*,1)*ns2p_fixed_found,factor_corrections_prv(*,2)*nop_fixed_found, max(x_grid),min(x_grid), 0.04D,yptsi_in_2dto1d,nel2dto1d,tec2dto1d)
...
p111=plot(x_grid,nsp_fixed_found,layout=[1,3,1],title='Aligned Fit in Black, Corrected It 1 in Red',ytitle='$n_{S^{+}}$ ($cm^{-3}$)',XTICKFORMAT="(A1)",margin=MARGINz0)
p1111=plot(x_grid,factor_corrections_prv(*,0)*nsp_fixed_found,/overplot,color='firebrick')
p111=plot(x_grid,ns2p_fixed_found,layout=[1,3,2],/current,ytitle='$n_{S^{++}}$ ($cm^{-3}$)',XTICKFORMAT="(A1)",margin=MARGINz1)
p1111=plot(x_grid,factor_corrections_prv(*,1)*ns2p_fixed_found,/overplot,color='firebrick')
p111=plot(x_grid,nop_fixed_found,layout=[1,3,3],/current,xtitle='Dusk $\rho_c$ ($R_J$)',ytitle='$n_{O^{+}}$ ($cm^{-3}$)',margin=MARGINz2)
p2111=plot(x_grid,factor_corrections_prv(*,2)*nop_fixed_found,/overplot,color='firebrick')

;p2111.save,'Aligned_Fit_vs_Corrected_It_1_densities.png',resolution=300
...
p1=plot(x_grid,model(*,2)/interpol(mean(reform(B_sim(*,*,2)),dimension = 1),rho_ceq,x_grid),NAME='$O^+$ 3729 Å',color="blue",xtitle='Dusk $\rho_c$ ($R_J$)',ytitle='Ratio',title='Model Aligned/(Model run through process), 2nd it',yrange=[1,2])
...
;leg2.save,'ratio_1.2x_correction_factor_dusk_model_aligned_over_process_2ndit.png',resolution=300


p1=plot(x_grid,interpol(mean(reform(B_sim(*,*,2)),dimension = 1),rho_ceq,x_grid),NAME='$O^+$ 3729 Å',color="blue",xtitle='Dusk $\rho_c$ ($R_J$)',ytitle='Ratio',title='Model Aligned/(Model run through process), same densities in eq.',yrange=[0,800])
...
;write_csv,'factor_corrections_dusk_2nd_iteration.csv',factor_corrections
