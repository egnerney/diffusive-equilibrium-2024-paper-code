...
;;;;;;;;;;;spacer
function calc_LOS_n_s_and_Tel_s_given_my_model_3D_cart_grid_difftwo, xgrid, ygrid, zgrid, norm_vec, slit_pos_vec, n_in, T_in, s=s, tel=tel
...
; Filter the unique s values, which are within the domain of the x, y, and z grid values.
s = ss[UNIQ(ss, SORT(ss))]
s = s[WHERE(s GE MIN(xgrid) AND s LE MAX(xgrid))]
...
Pro sim_citep_2_my_model_v1_diffeq_3D_cart_grid_given_emission_tables_8_APO_lines_make_movie_try_differenttwo
...
;tic
    ...
    n = calc_LOS_n_s_and_Tel_s_given_my_model_3D_cart_grid_difftwo( xgrid, ygrid, zgrid, norm_vec, slit_pos_vec, n_in, T_in, s=s, tel=tel)
...
; xrange1=[xmin,xmax]
  ...
  write_csv,'ypts_8apolinesa_vary_x0_and_z0_emission_table_4x279x129x8_movieframes_trydiff2.txt',ypts_vary_x0_and_z0
    write_csv,'ypts_total_8apolines_vary_x0_and_z0_emission_table_4x279x129_movieframes_trydiff2.txt',ypts_total_vary_x0_and_z0
