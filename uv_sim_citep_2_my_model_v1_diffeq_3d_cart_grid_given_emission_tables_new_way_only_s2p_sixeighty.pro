...
Function new_CITEP_2_given_n_s_and_tel_s_using_emission_tables_sum_sixeighty_s2p_line,s,n,Tel_s,fwhm,spec_binsize,yptsi_,nel_,tec_
...
Pro UV_sim_citep_2_my_model_v1_diffeq_3D_cart_grid_given_emission_tables_new_way_only_s2p_sixeighty
  ...
  idxxx = where((xwavi_in(s2p_idxs)  le 690) and (xwavi_in(s2p_idxs)  ge 670) )
 xwavi_in =xwavi_in(s2p_idxs)
 
 yptsi_in_temp = reform(yptsi_in(28,17,s2p_idxs))
  print,xwavi_in(idxxx)
  print, yptsi_in_temp(idxxx)/1d-6/1.0533062
  
 
  yptsi_in_s2p_sixeightyline = dblarr(61,57)
 ;stop
  ...
  for jjjj = 0, 57 -1  do begin ; to make single number to simulate total brightness because linearity of integrals
    ...
    yptsi_in_temp = reform(yptsi_in(iiii,jjjj,s2p_idxs))
    
    
    yptsi_in_s2p_sixeightyline(iiii,jjjj)  = total(yptsi_in_temp(idxxx),/double)
