...
def process_field_line(args):
    ...
    if (r_fl_ceq >= 4.00) and (r_fl_ceq <= 10.0):
        ...
        Aic = 2. #Ticnew / Ticold  # perp/par = kappa/max for fried egg
        Ahp = Aic #Thpnew / Thpold
        Aec = Aic#Tecnew / Tecold
        Aeh = Aic
        Aoph = Aic
        Ticnew = Ticnew/Aic
        Thpnew = Thpnew/Ahp
...
if __name__ == '__main__':
    ...
    species_lam = [1.] * nspec
    species_type = ['Aniso_Maxwellian'] * nspec#['Aniso_product_kappa'] * nspec
    ...
    np.savez_compressed('new_nominal_model_n_out_new_nominal_model_4-10_aniso2_max_all.npz', **n_out)

    # Save T_out (dictionary of species temperatures)
    np.savez_compressed('new_nominal_model_T_out_new_nominal_model_4-10_aniso2_max_all.npz', **T_out)

    # Save x_out, y_out, z_out, B_out (NumPy arrays)
    np.savez_compressed('new_nominal_model_field_data_new_nominal_model_4-10_aniso2_max_all.npz',
